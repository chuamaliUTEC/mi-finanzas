...
insert into public.financial_memory (user_id, memory_key, memory_value, confidence, source, status) values
  ...
  (:'user_id', 'prioridad_financiera', '{"texto": "1) Eliminar deuda cara. 2) Comprar departamento antes de los 30. 3) Construir patrimonio para el retiro a los 60. Manteniendo siempre una reserva mínima de liquidez."}', 0.9, 'usuario', 'actual'),
...
insert into public.financial_memory (user_id, memory_key, memory_value, confidence, source, status, effective_date) values
  ...
  (:'user_id', 'liquidez_ultima_conocida', '{"texto": "S/.150 disponibles (último dato conocido). NO usar como saldo actual sin confirmación."}', 0.3, 'usuario', 'desactualizado'),
  (:'user_id', 'gasto_mascota_referencia', '{"texto": "Referencia histórica: aprox. S/.28 por 2 kg de alimento cada ~3 días."}', 0.4, 'usuario', 'desactualizado'),
  (:'user_id', 'tarjeta_ultimo_estado_detalle', '{"texto": "Último registro conocido: línea S/.4,740, consumo S/.4,446, pago registrado S/.2,608 (equivalente USD 514.46 / aprox. S/.2,670). Requiere actualización para cálculos actuales."}', 0.5, 'usuario', 'desactualizado'),
  (:'user_id', 'prestamo_bancario_historico', '{"texto": "Plazo original 18 meses, cuota aproximada S/.430, se había pagado 1 cuota, vencimiento día 24. Saldo actual y tasa: por confirmar."}', 0.5, 'usuario', 'desactualizado'),
  (:'user_id', 'receivable_cop_historico', '{"texto": "Registros en COP sin nombre de deudor confirmado: COP 80,000; luego COP 150,000; menos COP 10,500 pagados = COP 139,500 pendientes según el último cálculo. Ambiguo: falta confirmar a quién corresponde y si son montos acumulativos o revisiones del mismo saldo antes de crear el registro formal en Cuentas por Cobrar."}', 0.3, 'usuario', 'desactualizado')
on conflict do nothing;
...
-- Deudas
-- ============================================================
-- interest_rate NULL = tasa por confirmar (nunca asumir 0 cuando se desconoce).
...
-- ============================================================
-- Tarjeta de crédito (último estado conocido, requiere actualización)
-- ============================================================
-- interest_rate NULL = tasa por confirmar (nunca asumir 0 cuando se desconoce).
insert into public.credit_cards (user_id, name, issuer, credit_limit, current_balance, interest_rate, currency, status_detail) values
  (:'user_id', 'Tarjeta principal', null, 4740, 4446, null, 'PEN', 'desactualizado')
on conflict do nothing;
